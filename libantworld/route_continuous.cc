...
//----------------------------------------------------------------------------
std::tuple<Vector2m, degree_t> RouteContinuous::getPosition(meter_t distance) const
{
    // Clamp distance at 0
    ...
    const Vector2m position(makeM(prevWaypoint[0] + proportion * (nextWaypoint[0] - prevWaypoint[0])),
                            makeM(prevWaypoint[1] + proportion * (nextWaypoint[1] - prevWaypoint[1])));

    // Return position
    return std::make_tuple(position, 90_deg + m_Headings[prevWaypointIndex]);
}
//----------------------------------------------------------------------------
