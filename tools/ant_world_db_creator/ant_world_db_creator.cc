...
protected:
    ...
    template<typename PoseType, typename RecordOp>
    void run(const PoseType &poses, RecordOp record)
    {
...
public:
    ...
    void runForGrid()
    {
        ...
        run(positions, [&gridRecorder](const cv::Mat &image) { gridRecorder.record(image); });
    }
};
...
public:
    ...
    void runForRoute()
    {
        ...
        run(poses, [&routeRecorder, this](const cv::Mat &image) {
            const auto &pose = m_Agent.pose();
            routeRecorder.record(pose.position(), pose.yaw(), image);
        });
    }
