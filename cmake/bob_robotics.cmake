...
macro(BoB_project)
    ...
    if(NOT TANK_TYPE)
        if(NOT "$ENV{TANK_TYPE}" STREQUAL "")
            ...
            set(TANK_TYPE Robots::Norbot)
        endif()
    endif()
    add_definitions(-DTANK_TYPE=${TANK_TYPE})
    message("Default tank robot type (if used): ${TANK_TYPE}")
    message("Default pose getter type: ${POSE_GETTER_TYPE}")

    # Extra modules needed for different tank types
    if(${TANK_TYPE} STREQUAL Robots::Norbot)
        set(TANK_HEADER "norbot.h")
    elseif(${TANK_TYPE} STREQUAL Robots::EV3)
        BoB_modules(robots/ev3)
        set(TANK_HEADER "ev3/ev3.h")
    elseif(${TANK_TYPE} STREQUAL Robots::BundledTankNetSink)
        BoB_modules(net)
        set(TANK_HEADER "tank_netsink.h")
    elseif(${TANK_TYPE} STREQUAL Robots::Gazebo::Tank)
        BoB_external_libraries(gazebo)
        BoB_modules(robots/gazebo)
        set(TANK_HEADER "gazebo/tank.h")
    elseif(${TANK_TYPE} STREQUAL AntWorld::Tank)
        BoB_modules(antworld)
        set(TANK_HEADER "antworld/tank.h")
    elseif(NOT ${TANK_TYPE} STREQUAL Robots::Tank)
        message(FATAL_ERROR "${TANK_TYPE} is not a recognised tank type")
    endif()
    ...
    if(NOT DEFINED POSE_GETTER_TYPE)
        set(POSE_GETTER_TYPE ${TANK_TYPE})
        add_definitions("-DPOSE_GETTER_TYPE=${POSE_GETTER_TYPE}*")
    elseif(${POSE_GETTER_TYPE} STREQUAL DummyPoseGetter)
        add_definitions("-DPOSE_GETTER_HEADER=\"common/dummy_pose_getter.h\"")
        add_definitions("-DPOSE_GETTER_TYPE=std::unique_ptr<${POSE_GETTER_TYPE}>")
        BoB_modules(common)
    elseif("${POSE_GETTER_TYPE}" STREQUAL Vicon::BundledVicon)
        add_definitions("-DPOSE_GETTER_HEADER=\"vicon/udp.h\"")
        add_definitions("-DPOSE_GETTER_TYPE=std::unique_ptr<${POSE_GETTER_TYPE}>")
        BoB_modules(vicon)
    endif()
    add_definitions("-DPOSE_GETTER_BASE_TYPE=${POSE_GETTER_TYPE}")
    string(TOUPPER ${POSE_GETTER_TYPE} POSE_GETTER_TYPE_UPPER)
    string(REPLACE :: _ POSE_GETTER_TYPE_UPPER ${POSE_GETTER_TYPE_UPPER})
    add_definitions(-DPOSE_GETTER_TYPE_${POSE_GETTER_TYPE_UPPER})
