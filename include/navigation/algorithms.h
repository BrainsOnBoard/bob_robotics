...
template<class It1, class It2, class Func, class... Ts>
void
...

{
    ...
    ImgProc::forEachRotation<float>(image, snapshotComp,
                                    std::forward<Ts>(columns)...);
}

template<class ImageType, class Container, class Differences, class Func, class... Ts>
void
                   ...
                   Ts&& ...columns)
{
    ...
    calculateRIDFMulti(image, std::cbegin(snapshots), std::cend(snapshots),
                       iterPair, calcSnapshotDifference,
                       std::forward<Ts>(columns)...);
}
              ...
              Func calcSnapshotDifference)
{
    ...
    calculateRIDFMulti(image, &snapshot, &snapshot + 1, &differences,
                       calcSnapshotDifference, createRange(image.cols));
    return differences;
}
