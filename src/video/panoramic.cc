...
std::unique_ptr<Input> getPanoramicCamera(cv::VideoCaptureAPIs api)
{
    ...
    // PixPro
        ...
        return std::make_unique<OpenCVInput>(deviceNum + api, cv::Size(1440, 1440), "pixpro_usb");
    }
    // Default
    else {
        return std::make_unique<OpenCVInput>(deviceNum + api, cv::Size(1280, 720), "webcam360");
    }
#endif
}
} // Video
