...
int main(int argc, char *argv[]) {
    ...
    my_drone.setRoomBounds(-1.5, 1.0, 
                           -1.0, 1.0, 
                           0.0, 2.0);
    ...
    while (true) {
        ...
        waypoint[0] = std::max(my_drone.m_RoomBounds.x[0], std::min(my_drone.m_RoomBounds.x[1], 
            waypoint[0] + (speed * js.getState(HID::JAxis::RightStickHorizontal))));
        waypoint[1] = std::max(my_drone.m_RoomBounds.y[0], std::min(my_drone.m_RoomBounds.y[1], 
            waypoint[1] - (speed * js.getState(HID::JAxis::RightStickVertical))));
        waypoint[2] = std::max(my_drone.m_RoomBounds.z[0], std::min(my_drone.m_RoomBounds.z[1],
            waypoint[2] - (speed * js.getState(HID::JAxis::LeftStickVertical))));
