...
int main(int argc, char *argv[]) {
  ...
  //wps.push_back({0.0,c_y,1.0,-90});

  /*wps.push_back({1.0,c_y,0.5,-90});
  wps.push_back({-1.0,c_y,0.5,-90});*/
  ...
  while (run) {
    ...
    if (pathOn == true) {
      ...
      if (!my_drone.hasQueuedWaypoint()) {
        my_drone.setNextWaypoint(wps[wp_ind][0],wps[wp_ind][1],wps[wp_ind][2],wps[wp_ind][3], false);
        wp_ind++;
    ...
    if (future.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
        ...
        if (line == "h") {
          my_drone.setWaypoint(0,c_y,0.5,CONST_YAW);
          my_drone.setNextWaypoint(0,c_y,0.5,CONST_YAW);
          controlOn = true;
          pathOn = false;
        }
        if (line == "l") {
          my_drone.setWaypoint(0.05,c_y,0,-90);
          my_drone.setNextWaypoint(0.05,c_y,0,-90);
          controlOn = true;
          pathOn = false;
        }
        if (line == "w") {
