...
#define VALIDATE(x, msg)                                                       \
    if (!(x)) {                                                                \
        MessageBoxA(nullptr, (msg), "OculusRoomTiny", MB_ICONERROR | MB_OK);   \
        exit(-1);                                                              \
    }
#endif
...
struct DataBuffer
{
    ...
    DataBuffer(ID3D11Device *Device,
               D3D11_BIND_FLAG use,
               const void *buffer,
               size_t size)
      : Size(size)
...
struct DirectX11
{
                    ...
                    int scale = 1)
    {
        ...
        for (UINT iAdapter = 0; DXGIFactory->EnumAdapters(iAdapter, &Adapter) !=
                                DXGI_ERROR_NOT_FOUND;
             ++iAdapter) {
            DXGI_ADAPTER_DESC adapterDesc;
            Adapter->GetDesc(&adapterDesc);
            if ((pLuid == nullptr) ||
                memcmp(&adapterDesc.AdapterLuid, pLuid, sizeof(LUID)) == 0)
                break;
            Release(Adapter);
        }
        ...
        // Still hit an error even after installing
        // Windows Kit 8 includes it:
        ...
        UniformBufferGen = new DataBuffer(
                Device, D3D11_BIND_CONSTANT_BUFFER, nullptr, UNIFORM_DATA_SIZE);
        Context->VSSetConstantBuffers(0, 1, &UniformBufferGen->D3DBuffer);

        // Set max frame latency to 1
        IDXGIDevice1 *DXGIDevice1 = nullptr;
        hr = Device->QueryInterface(__uuidof(IDXGIDevice1),
                                    (void **) &DXGIDevice1);
    ...
    void SetAndClearRenderTarget(ID3D11RenderTargetView *rendertarget,
                                 ...
                                 float A = 1.0f)
    {
        SetAndClearRenderTarget(rendertarget,
                                depthbuffer
                                        ? depthbuffer->TexDsv
    ...
    void SetAndClearRenderTarget(ID3D11RenderTargetView *rendertarget,
                                 ...
                                 float A = 1.0f)
    {
        float black[] = { R, G, B, A }; // Important that alpha=0, if want
                                        // pixels to be transparent, for manual
                                        // layers
...
// to facilitate automated testing
    ...
    void Run(bool (*MainLoop)(bool retryCreate))
    {
        while (HandleMessages()) {
            ...
            if (!MainLoop(true))
                break;
            // Sleep a bit before retrying to reduce CPU load while the HMD is
            // disconnected
            Sleep(10);
        }
    }
...
struct Texture
{
              ...
              int sampleCount)
    {
        ...
        TexRtv = nullptr;
        if (rendertarget)
            DIRECTX.Device->CreateRenderTargetView(Tex, nullptr, &TexRtv);
    }
    Texture(int sizeW,
            int sizeH,
            bool rendertarget,
    ...
    void FillTexture(uint32_t *pix)
    {
        ...
        for (int level = 0; level < MipLevels; level++) {
            DIRECTX.Context->UpdateSubresource(Tex,
                                               level,
                                               nullptr,
                                               (unsigned char *) pix,
                                               sizeW * 4,
            ...
            for (int j = 0; j < (sizeH & ~1); j += 2) {
                ...
                for (int i = 0; i<sizeW>> 1; i++, psrc += 8, pdest += 4) {
                    pdest[0] = (((int) psrc[0]) + psrc[4] +
                                psrc[sizeW * 4 + 0] + psrc[sizeW * 4 + 4]) >>
                               2;
                    pdest[1] = (((int) psrc[1]) + psrc[5] +
                                psrc[sizeW * 4 + 1] + psrc[sizeW * 4 + 5]) >>
                               2;
                    pdest[2] = (((int) psrc[2]) + psrc[6] +
                                psrc[sizeW * 4 + 2] + psrc[sizeW * 4 + 6]) >>
                               2;
                    pdest[3] = (((int) psrc[3]) + psrc[7] +
                                psrc[sizeW * 4 + 3] + psrc[sizeW * 4 + 7]) >>
                               2;
                }
            }
            sizeW >>= 1;
    ...
    static void ConvertToSRGB(uint32_t *linear)
    {
        ...
        *linear = (*linear & 0xff000000) + (drgb[2] << 16) + (drgb[1] << 8) +
                  (drgb[0] << 0);
    }
            ...
            for (int i = 0; i < SizeW; i++) {
                ...
                case (AUTO_WALL):
                    *curr = (((j / 4 & 15) == 0) ||
                    ...
                    break;
                case (AUTO_FLOOR):
                    *curr = (((i >> 7) ^ (j >> 7)) & 1) ? 0xffb4b4b4
                                                        : 0xff505050;
                    break;
                case (AUTO_CEILING):
                    *curr = (i / 4 == 0 || j / 4 == 0) ? 0xff505050
                                                       : 0xffb4b4b4;
                    break;
                case (AUTO_WHITE):
                    *curr = 0xffffffff;
...
struct Material
{
      ...
      , VertexSize(vSize)
    {
        ...
        char *defaultVertexShaderSrc =
                (char *) "float4x4 ProjView;  float4 MasterCol;"
                       "void main(in  float4 Position  : POSITION,    in  "
                       "float4 Color : COLOR0, in  float2 TexCoord  : "
                       "TEXCOORD0,"
                       "          out float4 oPosition : SV_Position, out "
                       "float4 oColor: COLOR0, out float2 oTexCoord : "
                       "TEXCOORD0)"
                       "{   oPosition = mul(ProjView, Position); oTexCoord = "
                       "TexCoord; "
                       "    oColor = MasterCol * Color; }";
        char *defaultPixelShaderSrc =
                (char *) "Texture2D Texture   : register(t0); SamplerState "
                         "Linear : "
                         "register(s0); "
                         "float4 main(in float4 Position : SV_Position, in "
                         "float4 "
                         "Color: COLOR0, in float2 TexCoord : TEXCOORD0) : "
                         "SV_Target"
                         "{   float4 TexCol = Texture.Sample(Linear, "
                         "TexCoord); "
                         "    if (TexCol.a==0) clip(-1); " // If alpha = 0,
                                                           // don't draw
                         "    return(Color * TexCol); }";

        // vertex shader for instanced stereo
        char *instancedStereoVertexShaderSrc =
                (char *) "float4x4 modelViewProj[2];  float4 MasterCol;"
                         "void main(in  float4 Position  : POSITION,    in  "
                         "float4 "
                         "Color : COLOR0, in  float2 TexCoord  : TEXCOORD0,"
                         ...
                         "          out float4 oPosition : SV_Position, out "
                         "float4 "
                         "oColor: COLOR0, out float2 oTexCoord : TEXCOORD0,"
                         "          out float oClipDist : SV_ClipDistance0, "
                         "out float "
                         "oCullDist : SV_CullDistance0)"
                         "{"
                         "   const float4 EyeClipPlane[2] = { { -1, 0, 0, 0 }, "
                         "{ 1, 0, "
                         "0, 0 } };"
                         "   uint eyeIndex = instanceID & 1;"
                         // transform to clip space for correct eye (includes
                         // offset and scale)
                         "   oPosition = mul(modelViewProj[eyeIndex], "
                         "Position); "
                         // calculate distance from left/right clip plane (try
                         // setting to 0 to see why clipping is necessary)
                         "   oCullDist = oClipDist = "
                         "dot(EyeClipPlane[eyeIndex], "
                         "oPosition);"
                         "   oTexCoord = TexCoord; "
                         "   oColor = MasterCol * Color;"
                         "}";
        ...
        if (!vertexDesc)
            vertexDesc = defaultVertexDesc;
        if (!vertexShader)
            vertexShader = defaultVertexShaderSrc;
        if (!pixelShader)
            pixelShader = defaultPixelShaderSrc;

        // Create vertex shader
        ID3DBlob *blobData;
        ID3DBlob *errorBlob = nullptr;
        HRESULT result = D3DCompile(vertexShader,
                                    strlen(vertexShader),
                                    0,
                                    0,
                                    0,
        ...
        DIRECTX.Device->CreateVertexShader(blobData->GetBufferPointer(),
                                           blobData->GetBufferSize(),
                                           nullptr,
                                           &VertexShader);

        // Create input layout
        DIRECTX.Device->CreateInputLayout(vertexDesc,
                                          numVertexDesc,
        ...
        result = D3DCompile(instancedStereoVertexShaderSrc,
                            strlen(instancedStereoVertexShaderSrc),
                            0,
                            0,
                            0,
                            "main",
        ...
        D3DCompile(pixelShader,
                   ...
                   &blobData,
                   0);
        DIRECTX.Device->CreatePixelShader(blobData->GetBufferPointer(),
                                          blobData->GetBufferSize(),
                                          nullptr,
                                          &PixelShader);
        blobData->Release();
        ...
        ss.AddressU = ss.AddressV = ss.AddressW =
                flags & MAT_WRAP ? D3D11_TEXTURE_ADDRESS_WRAP
        ...
        rs.FillMode =
                flags & MAT_WIRE ? D3D11_FILL_WIREFRAME : D3D11_FILL_SOLID;
        ...
        dss.DepthFunc = flags & MAT_ZALWAYS ? D3D11_COMPARISON_ALWAYS
        ...
        bm.RenderTarget[0].BlendOp = bm.RenderTarget[0].BlendOpAlpha =
                D3D11_BLEND_OP_ADD;
        bm.RenderTarget[0].SrcBlend = bm.RenderTarget[0].SrcBlendAlpha =
...
#define GetLengthLocal(v) (sqrt(v.x * v.x + v.y * v.y + v.z * v.z))
        float dist1 = GetLengthLocal(
                XMFLOAT3(pos.x - (-2), pos.y - (4), pos.z - (-2)));
        float dist2 = GetLengthLocal(
                XMFLOAT3(pos.x - (3), pos.y - (4), pos.z - (-3)));
        float dist3 = GetLengthLocal(
                XMFLOAT3(pos.x - (-4), pos.y - (3), pos.z - (25)));
        int bri = rand() % 160;
        float R = ((c >> 16) & 0xff) *
                  (bri + 192.0f * (0.65f + 8 / dist1 + 1 / dist2 + 4 / dist3)) /
                  255.0f;
        float G = ((c >> 8) & 0xff) *
                  (bri + 192.0f * (0.65f + 8 / dist1 + 1 / dist2 + 4 / dist3)) /
                  255.0f;
        float B = ((c >> 0) & 0xff) *
                  (bri + 192.0f * (0.65f + 8 / dist1 + 1 / dist2 + 4 / dist3)) /
                  255.0f;
        return ((c & 0xff000000) + ((R > 255 ? 255 : (uint32_t) R) << 16) +
                ((G > 255 ? 255 : (uint32_t) G) << 8) +
                (B > 255 ? 255 : (uint32_t) B));
    }

    void AddSolidColorBox(float x1,
                          float y1,
                          float z1,
                          float x2,
                          float y2,
                          ...
                          uint32_t c)
    {
        AddQuad(Vertex(XMFLOAT3(x1, y2, z1),
                       ModifyColor(c, XMFLOAT3(x1, y2, z1)),
                       z1,
                       x1),
                Vertex(XMFLOAT3(x2, y2, z1),
                       ModifyColor(c, XMFLOAT3(x2, y2, z1)),
                       z1,
                       x2),
                Vertex(XMFLOAT3(x1, y2, z2),
                       ModifyColor(c, XMFLOAT3(x1, y2, z2)),
                       z2,
                       x1),
                Vertex(XMFLOAT3(x2, y2, z2),
                       ModifyColor(c, XMFLOAT3(x2, y2, z2)),
                       z2,
                       x2));
        AddQuad(Vertex(XMFLOAT3(x2, y1, z1),
                       ModifyColor(c, XMFLOAT3(x2, y1, z1)),
                       z1,
                       x2),
                Vertex(XMFLOAT3(x1, y1, z1),
                       ModifyColor(c, XMFLOAT3(x1, y1, z1)),
                       z1,
                       x1),
                Vertex(XMFLOAT3(x2, y1, z2),
                       ModifyColor(c, XMFLOAT3(x2, y1, z2)),
                       z2,
                       x2),
                Vertex(XMFLOAT3(x1, y1, z2),
                       ModifyColor(c, XMFLOAT3(x1, y1, z2)),
                       z2,
                       x1));
        AddQuad(Vertex(XMFLOAT3(x1, y1, z2),
                       ModifyColor(c, XMFLOAT3(x1, y1, z2)),
                       z2,
                       y1),
                Vertex(XMFLOAT3(x1, y1, z1),
        ...
        AddQuad(Vertex(XMFLOAT3(x2, y1, z1),
                ...
                Vertex(XMFLOAT3(x2, y2, z2),
                       ...
                       z2,
                       y2));
        AddQuad(Vertex(XMFLOAT3(x1, y1, z1),
                       ModifyColor(c, XMFLOAT3(x1, y1, z1)),
                       x1,
                       y1),
                Vertex(XMFLOAT3(x2, y1, z1),
                       ModifyColor(c, XMFLOAT3(x2, y1, z1)),
                       x2,
                       y1),
                Vertex(XMFLOAT3(x1, y2, z1),
                ...
                Vertex(XMFLOAT3(x2, y2, z1),
                       ...
                       x2,
                       y2));
        AddQuad(Vertex(XMFLOAT3(x2, y1, z2),
                       ModifyColor(c, XMFLOAT3(x2, y1, z2)),
                       x2,
                       y1),
                Vertex(XMFLOAT3(x1, y1, z2),
...
struct Model
{
    ...
    int NumIndices;
    bool IsShown; // flag to indicate whether this model is rendered. Default is
                  // true.
    ...
    void Init(TriangleSet *t)
    {
        ...
        VertexBuffer = new DataBuffer(DIRECTX.Device,
                                      D3D11_BIND_VERTEX_BUFFER,
                                      &t->Vertices[0],
                                      t->numVertices * sizeof(Vertex));
        IndexBuffer = new DataBuffer(DIRECTX.Device,
                                     D3D11_BIND_INDEX_BUFFER,
                                     &t->Indices[0],
                                     t->numIndices * sizeof(short));
    }
                ...
                bool standardUniforms)
    {
        if (IsShown) {
            ...
            DIRECTX.Context->Map(DIRECTX.UniformBufferGen->D3DBuffer,
                                 0,
            ...
            DIRECTX.Context->IASetIndexBuffer(
                    IndexBuffer->D3DBuffer, DXGI_FORMAT_R16_UINT, 0);
            UINT offset = 0;
            DIRECTX.Context->IASetVertexBuffers(
                    0, 1, &VertexBuffer->D3DBuffer, &Fill->VertexSize, &offset);
            DIRECTX.Context->IASetPrimitiveTopology(
            ...
            DIRECTX.Context->OMSetBlendState(
                    Fill->BlendState, nullptr, 0xffffffff);
                         ...
                         bool standardUniforms)
    {
        XMMATRIX modelMat = XMMatrixMultiply(
                ...
                XMMatrixRotationQuaternion(XMLoadFloat4(&Rot)));
        modelMat = XMMatrixMultiply(
                modelMat, XMMatrixTranslationFromVector(XMLoadFloat3(&Pos)));
        XMMATRIX modelViewProjMatL =
                XMMatrixMultiply(modelMat, viewProjMats[0]);
        XMMATRIX modelViewProjMatR =
                XMMatrixMultiply(modelMat, viewProjMats[1]);
        float col[] = { R, G, B, A };
        if (standardUniforms) {
...
struct Scene
{
    ...
    {
        TriangleSet cube;
        cube.AddSolidColorBox(
                0.5f, -0.5f, 0.5f, -0.5f, 0.5f, -0.5f, 0xff404040);
        Add(new Model(&cube,
                      XMFLOAT3(0, 0, 0),
                      XMFLOAT4(0, 0, 0, 1),
                      new Material(new Texture(
                              false, 256, 256, Texture::AUTO_CEILING))));

        TriangleSet spareCube;
        spareCube.AddSolidColorBox(
                0.1f, -0.1f, 0.1f, -0.1f, +0.1f, -0.1f, 0xffff0000);
        Add(new Model(&spareCube,
                      XMFLOAT3(0, -10, 0),
                      XMFLOAT4(0, 0, 0, 1),
                      new Material(new Texture(
                              false, 256, 256, Texture::AUTO_CEILING))));
        ...
        TriangleSet walls;
        walls.AddSolidColorBox(10.1f,
                               0.0f,
                               20.0f,
                               10.0f,
                               4.0f,
                               -20.0f,
                               0xff808080); // Left Wall
        walls.AddSolidColorBox(10.0f,
                               -0.1f,
                               20.1f,
                               -10.0f,
                               4.0f,
                               20.0f,
                               0xff808080); // Back Wall
        walls.AddSolidColorBox(-10.0f,
                               -0.1f,
                               20.0f,
                               -10.1f,
        ...
        Add(new Model(&walls,
                      ...
                      XMFLOAT4(0, 0, 0, 1),
                      new Material(new Texture(
                              false, 256, 256, Texture::AUTO_WALL))));

        if (includeIntensiveGPUobject) {
            TriangleSet partitions;
            for (float depth = 0.0f; depth > -3.0f; depth -= 0.1f)
                partitions.AddSolidColorBox(9.0f,
                                            0.5f,
                                            -depth,
                                            -9.0f,
                                            3.5f,
                                            -depth,
                                            0x10ff80ff); // Partition
            Add(new Model(
                    &partitions,
                    XMFLOAT3(0, 0, 0),
                    XMFLOAT4(0, 0, 0, 1),
                    new Material(new Texture(
                            false, 256, 256, Texture::AUTO_FLOOR)))); // Floors
        }

        TriangleSet floors;
        floors.AddSolidColorBox(10.0f,
                                -0.1f,
                                20.0f,
                                -10.0f,
                                0.0f,
        ...
        TriangleSet ceiling;
        ceiling.AddSolidColorBox(
                10.0f, 4.0f, 20.0f, -10.0f, 4.1f, -20.1f, 0xff808080);
        Add(new Model(
                &ceiling,
                XMFLOAT3(0, 0, 0),
                XMFLOAT4(0, 0, 0, 1),
                new Material(new Texture(
                        false, 256, 256, Texture::AUTO_CEILING)))); // Ceiling

        TriangleSet furniture;
        furniture.AddSolidColorBox(-9.5f,
                                   0.75f,
                                   -3.0f,
                                   -10.1f,
                                   2.5f,
        ...
        furniture.AddSolidColorBox(-9.55f,
                                   2.00f,
                                   -3.05f,
                                   -10.1f,
                                   2.10f,
                                   -4.2f,
                                   0xff383838); // Right side shelf
        furniture.AddSolidColorBox(-5.0f,
                                   1.1f,
                                   -20.0f,
                                   -10.0f,
                                   1.2f,
        ...
        furniture.AddSolidColorBox(10.0f,
                                   1.1f,
                                   -20.0f,
                                   5.0f,
                                   1.2f,
        ...
        furniture.AddSolidColorBox(1.401f,
                                   0.0f,
                                   1.101f,
                                   1.339f,
                                   1.0f,
        ...
        furniture.AddSolidColorBox(0.799f,
                                   0.0f,
                                   0.499f,
                                   0.861f,
                                   0.5f,
        ...
        furniture.AddSolidColorBox(0.799f,
                                   ...
                                   1.039f,
                                   0xff202050); // Chair Leg 2
        furniture.AddSolidColorBox(1.4f,
                                   0.97f,
                                   1.05f,
                                   0.8f,
                                   0.92f,
        ...
        for (float f = 3.0f; f <= 6.6f; f += 0.4f)
            furniture.AddSolidColorBox(
                    3, 0.0f, -f, 2.9f, 1.3f, -f - 0.1f, 0xff404040); // Posts
        Add(new Model(&furniture,
                      XMFLOAT3(0, 0, 0),
                      XMFLOAT4(0, 0, 0, 1),
                      new Material(new Texture(
                              false,
