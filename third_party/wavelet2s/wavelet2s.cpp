...
void *
per_ext2d(vector<vector<double>> &signal, vector<vector<double>> &temp2, int a)
{
...
void *
swt_2d(vector<vector<double>> &sig, int J, string nm, vector<double> &swt_output)
{
...
void *
per_ext(vector<double> &sig, int a)
{
    unsigned int len;
    len = sig.size();
    if ((len % 2) != 0) {
...
void *
iswt(vector<double> &swtop, int J, string nm, vector<double> &iswt_output)
{
...
void *
swt(vector<double> &signal1, int J, string nm, vector<double> &swt_output, int &length)
{
    vector<double> lpd, hpd, lpr, hpr;
...
void *
dwt_output_dim_sym(vector<int> &length, vector<int> &length2, int J)
{
...
void *
dwt_output_dim2(vector<int> &length, vector<int> &length2, int J)
{
...
void
symm_ext2d(vector<vector<double>> &signal, vector<vector<double>> &temp2, int a)
{
    unsigned int rows = signal.size();
    unsigned int cols = signal[0].size();
...
void *
circshift2d(vector<vector<double>> &signal, int x, int y)
{
    unsigned int rows = signal.size();
    unsigned int cols = signal[0].size();
...
void *
idwt_2d_sym(vector<double> &dwtop, vector<double> &flag, string nm, vector<vector<double>> &idwt_output, vector<int> &length)
{
    int J = (int) flag[0];
    int rows = length[0];
    int cols = length[1];
...
void *
dwt2_sym(string name, vector<vector<double>> &signal, vector<vector<double>> &cLL, vector<vector<double>> &cLH, vector<vector<double>> &cHL, vector<vector<double>> &cHH)
{
    //Analysis
...
void *
dwt_2d_sym(vector<vector<double>> &origsig, int J, string nm, vector<double> &dwt_output, vector<double> &flag, vector<int> &length)
{
    // flag will contain
...
void *
idwt1_sym(string wname, vector<double> &X, vector<double> &app, vector<double> &detail)
{
...
void *
idwt1_sym_m(string wname, vector<double> &idwt_output, vector<double> &app, vector<double> &detail)
{
...
void *
symm_ext(vector<double> &sig, int a)
{
...
void *
idwt_sym(vector<double> &dwtop, vector<double> &flag, string nm, vector<double> &idwt_output, vector<int> &length)
{
...
void *
dwt1_sym(string wname, vector<double> &signal, vector<double> &cA, vector<double> &cD)
{
...
void *
dwt1_sym_m(string wname, vector<double> &signal, vector<double> &cA, vector<double> &cD)
{
...
void *
freq(vector<double> &sig, vector<double> &freq_resp)
{
...
double
convfft(vector<double> &a, vector<double> &b, vector<double> &c)
{
...
double
convfftm(vector<double> &a, vector<double> &b, vector<double> &c)
{
    fftw_complex *inp_data, *filt_data, *inp_fft, *filt_fft, *temp_data, *temp_ifft;
...
void *
fft(vector<complex<double>> &data, int sign, unsigned int N)
{
...
void *
bitreverse(vector<complex<double>> &sig)
{
    unsigned int len = sig.size();
...
void *
dwt(vector<double> &sig, int J, string nm, vector<double> &dwt_output, vector<double> &flag, vector<int> &length)
{
...
void
circshift(vector<double> &sig_cir, int L)
{
...
double
convol(vector<double> &a1, vector<double> &b1, vector<double> &c)
{
...
void
downsamp(vector<double> &sig, int M, vector<double> &sig_d)
{
    int len = sig.size();
...
void *
dwt1(string wname, vector<double> &signal, vector<double> &cA, vector<double> &cD)
{
...
void *
dwt1_m(string wname, vector<double> &signal, vector<double> &cA, vector<double> &cD)
{
...
void *
dyadic_zpad_1d(vector<double> &signal)
{
...
void *
idwt(vector<double> &dwtop, vector<double> &flag, string nm, vector<double> &idwt_output, vector<int> &length)
{
...
void *
idwt1_m(string wname, vector<double> &X, vector<double> &cA, vector<double> &cD)
{
    vector<double> lpd1, hpd1, lpr1, hpr1;
...
void *
idwt1(string wname, vector<double> &X, vector<double> &cA, vector<double> &cD)
{
    vector<double> lpd1, hpd1, lpr1, hpr1;
...
void
upsamp(vector<double> &sig, int M, vector<double> &sig_u)
{
    int len = sig.size();
    double len_n = ceil((double) len * (double) M);
...
double
op_sum(double i, double j)
{
    return (i + j);
}

int
vecsum(vector<double> &a, vector<double> &b, vector<double> &c)
{
...
void *
getcoeff2d(vector<vector<double>> &dwtoutput, vector<vector<double>> &cH, vector<vector<double>> &cV, vector<vector<double>> &cD, vector<double> &flag, int &N)
{
    if (N > flag[2]) {
...
void *
zero_remove(vector<vector<double>> &input, vector<vector<double>> &output)
{
...
void *
dwt_output_dim(vector<vector<double>> &signal, int &r, int &c)
{
    int rows = signal.size();
    int cols = signal[0].size();
...
void *
dyadic_zpad_2d(vector<vector<double>> &signal, vector<vector<double>> &mod)
{
    int rows = signal.size();
    int cols = signal[0].size();
...
void *
idwt_2d(vector<double> &dwtop, vector<double> &flag, string nm, vector<vector<double>> &idwt_output, vector<int> &length)
{
    int J = (int) flag[0];
    int rows = length[0];
    int cols = length[1];
...
void *
dwt_2d(vector<vector<double>> &origsig, int J, string nm, vector<double> &dwt_output, vector<double> &flag, vector<int> &length)
{
    ...
    // Calculate signal dimensions to check if level J can be computed
    Max_Iter = min((int) ceil(log(double(sig.size())) / log(2.0)), (int) ceil(log(double(sig[0].size())) / log(2.0)));
    if (Max_Iter < J) {
        cout << J << " Iterations are not possible with signals of this dimension " << endl;
        exit(1);
    ...
    // Store the level use in the flag variable
    flag.push_back(double(J));
    flag.push_back(0);

    // store the number of columns and rows into the length variable.
    // This is need to retrieve the coeffs in the output container
    length.insert(length.begin(), cols_n);
    length.insert(length.begin(), rows_n);

    int sum_coef = 0;

    // For each Level J of the Decomposition ...
    for (int iter = 0; iter < J; iter++) {
        // ... get filtcoeffs depending on selected wavelet/scaling-function ...
        filtcoef(nm, lp1, hp1, lp2, hp2);

        // ... calculate the size after downsampling by two ...
        rows_n = (int) ceil((double) rows_n / 2.0);
        cols_n = (int) ceil((double) cols_n / 2.0);

        // ... push the new size info into the length container...
        length.insert(length.begin(), cols_n);
        length.insert(length.begin(), rows_n);

        // ... create container variables to store calculated coeffs...
        vector<vector<double>> cA(rows_n, vector<double>(cols_n));
        vector<vector<double>> cH(rows_n, vector<double>(cols_n));
        vector<vector<double>> cV(rows_n, vector<double>(cols_n));
        vector<vector<double>> cD(rows_n, vector<double>(cols_n));
        ...
        if (iter == 0) {
            ...
            // ... use dwt2 function as well (???)...
            dwt2(nm, original_copy, cA, cH, cV, cD);
        }

        // ... create container for downsampled signal for next step ...
        vector<double> temp_sig2;

        // ... the signal used for the next decomposition level is the approximation of this level ...
        original_copy = cA;

        // ... if this is the last level of decomposition ...
        if (iter == J - 1) {
            for (int i = 0; i < rows_n; i++) {
                for (int j = 0; j < cols_n; j++) {
                    // ... save all  approximation values (the signal at this level) into the container
                    // because the first value in the output vector is going to be the approximation at level J ...
                    double temp = cA[i][j];
                    temp_sig2.push_back(temp);
                }
            }
        ...
        /*
                The below part is repeated for all coeffs
                */

        // ... for each row and column ...
        for (int i = 0; i < rows_n; i++) {
            for (int j = cols_n; j < cols_n * 2; j++) {
                // ... get the coeff for this row and colunmn
                double temp = cH[i][j - cols_n];
                temp_sig2.push_back(temp);
            }
        }
        ...
        // ... push the results of this level into the final output...
        dwt_output.insert(dwt_output.begin(), temp_sig2.begin(), temp_sig2.end());

        // ... keep track of the number of coefficients
        sum_coef += 4 * rows_n * cols_n;
    }
    /*
                        ofstream dwt2out("dwt2out.dat");
...
void *
branch_lp_hp_up(string wname, vector<double> &cA, vector<double> &cD, vector<double> &X)
{
    vector<double> lpd1, hpd1, lpr1, hpr1;
...
void *
branch_hp_dn(string wname, vector<double> &signal, vector<double> &sigop)
{
...
void *
branch_lp_dn(string wname, vector<double> &signal, vector<double> &sigop)
{
...
void *
idwt2(string name, vector<vector<double>> &signal, vector<vector<double>> &cLL, vector<vector<double>> &cLH, vector<vector<double>> &cHL, vector<vector<double>> &cHH)
{
    // Synthesis
    int rows = cLL.size();
    int cols = cLL[0].size();
...
void *
dwt2(string name, vector<vector<double>> &signal, vector<vector<double>> &cLL, vector<vector<double>> &cLH, vector<vector<double>> &cHL, vector<vector<double>> &cHH)
{
    //Analysis
    int rows = signal.size();
    int cols = signal[0].size();
...
void *
downsamp2(vector<vector<double>> &vec1, vector<vector<double>> &vec2, int rows_dn, int cols_dn)
{
...
void *
upsamp2(vector<vector<double>> &vec1, vector<vector<double>> &vec2, int rows_up, int cols_up)
{
...
int
filtcoef(string name, vector<double> &lp1, vector<double> &hp1, vector<double> &lp2, vector<double> &hp2)
{
    if (name == "haar" || name == "db1") {
        ...
        double lp1_a[] = { -0.12940952255092145, 0.22414386804185735, 0.83651630373746899, 0.48296291314469025 };
        lp1.assign(lp1_a, lp1_a + sizeof(lp1_a) / sizeof(double));

        double hp1_a[] = { -0.48296291314469025, 0.83651630373746899, -0.22414386804185735, -0.12940952255092145 };
        hp1.assign(hp1_a, hp1_a + sizeof(hp1_a) / sizeof(double));

        double lp2_a[] = { 0.48296291314469025, 0.83651630373746899, 0.22414386804185735, -0.12940952255092145 };
        lp2.assign(lp2_a, lp2_a + sizeof(lp2_a) / sizeof(double));

        double hp2_a[] = { -0.12940952255092145, -0.22414386804185735, 0.83651630373746899, -0.48296291314469025 };
        hp2.assign(hp2_a, hp2_a + sizeof(hp2_a) / sizeof(double));
        return 0;
    }

    else if (name == "db3") {
        double lp1_a[] = { 0.035226291882100656, -0.085441273882241486, -0.13501102001039084, 0.45987750211933132, 0.80689150931333875, 0.33267055295095688 };
        lp1.assign(lp1_a, lp1_a + sizeof(lp1_a) / sizeof(double));

        double hp1_a[] = { -0.33267055295095688, 0.80689150931333875, -0.45987750211933132, -0.13501102001039084, 0.085441273882241486, 0.035226291882100656 };
        hp1.assign(hp1_a, hp1_a + sizeof(hp1_a) / sizeof(double));
        ...
        double hp2_a[] = { 0.035226291882100656, 0.085441273882241486, -0.13501102001039084, -0.45987750211933132, 0.80689150931333875, -0.33267055295095688 };
        hp2.assign(hp2_a, hp2_a + sizeof(hp2_a) / sizeof(double));
        return 0;
    }

    else if (name == "db4") {
        double lp1_a[] = { -0.010597401784997278, 0.032883011666982945, 0.030841381835986965, -0.18703481171888114, -0.027983769416983849, 0.63088076792959036, 0.71484657055254153, 0.23037781330885523 };
        lp1.assign(lp1_a, lp1_a + sizeof(lp1_a) / sizeof(double));

        double hp1_a[] = { -0.23037781330885523, 0.71484657055254153, -0.63088076792959036, -0.027983769416983849, 0.18703481171888114, 0.030841381835986965, -0.032883011666982945, -0.010597401784997278 };
        hp1.assign(hp1_a, hp1_a + sizeof(hp1_a) / sizeof(double));

        double lp2_a[] = { 0.23037781330885523, 0.71484657055254153, 0.63088076792959036, -0.027983769416983849, -0.18703481171888114, 0.030841381835986965, 0.032883011666982945, -0.010597401784997278 };
        lp2.assign(lp2_a, lp2_a + sizeof(lp2_a) / sizeof(double));

        double hp2_a[] = { -0.010597401784997278, -0.032883011666982945, 0.030841381835986965, 0.18703481171888114, -0.027983769416983849, -0.63088076792959036, 0.71484657055254153, -0.23037781330885523 };
        hp2.assign(hp2_a, hp2_a + sizeof(hp2_a) / sizeof(double));
        return 0;
    }

    else if (name == "db5") {
        double lp1_a[] = { 0.0033357252850015492, -0.012580751999015526, -0.0062414902130117052, 0.077571493840065148, -0.03224486958502952, -0.24229488706619015, 0.13842814590110342, 0.72430852843857441, 0.60382926979747287, 0.16010239797412501 };
        lp1.assign(lp1_a, lp1_a + sizeof(lp1_a) / sizeof(double));

        double hp1_a[] = { -0.16010239797412501, 0.60382926979747287, -0.72430852843857441, 0.13842814590110342, 0.24229488706619015, -0.03224486958502952, -0.077571493840065148, -0.0062414902130117052, 0.012580751999015526, 0.0033357252850015492 };
        hp1.assign(hp1_a, hp1_a + sizeof(hp1_a) / sizeof(double));

        double lp2_a[] = { 0.16010239797412501, 0.60382926979747287, 0.72430852843857441, 0.13842814590110342, -0.24229488706619015, -0.03224486958502952, 0.077571493840065148, -0.0062414902130117052, -0.012580751999015526, 0.0033357252850015492 };
        lp2.assign(lp2_a, lp2_a + sizeof(lp2_a) / sizeof(double));

        double hp2_a[] = { 0.0033357252850015492, 0.012580751999015526, -0.0062414902130117052, -0.077571493840065148, -0.03224486958502952, 0.24229488706619015, 0.13842814590110342, -0.72430852843857441, 0.60382926979747287, -0.16010239797412501 };
        hp2.assign(hp2_a, hp2_a + sizeof(hp2_a) / sizeof(double));
        return 0;
    }
